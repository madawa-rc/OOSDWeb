...
//        }
    ...
    public static School getSchool(String email) {
        ...
        try {
            ...
            slomf.admin.Log.addLog("School gotten from DB");
        } catch (SQLException ex) {
            slomf.admin.Log.addLog(ex.getMessage());
        }
        return s;
    ...
    public static ArrayList<Student> getStudents(School school) {
        ...
        try {
            ...
            //      ps.setString(2, "0");
            ResultSet rs = ps.executeQuery();
         //   slomf.admin.Log.addLog("Finding students " + school.getName());
            while (rs.next()) {
            ...
            if (studentList.isEmpty()) {
                slomf.admin.Log.addLog("Empty Student List " + school.getName());
            } else {
                slomf.admin.Log.addLog("Got Student List " + school.getName());
            }
        } catch (SQLException ex) {
            slomf.admin.Log.addLog(ex.getMessage());
        }
    ...
    public static ArrayList<School> getAllSchools(String searchString) {
        slomf.admin.Log.addLog(searchString);
        ArrayList<School> schoolList = new ArrayList<School>();
        try {
            Connection con = DatabaseConnectionHandler.createConnection();
